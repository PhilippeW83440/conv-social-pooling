...
class highwayNet(nn.Module):
	...
	def forward(self,hist,nbrs,masks,lat_enc,lon_enc, hist_grid=None, fut=None):
		...
		if self.use_transformer is False or (self.use_transformer and self.use_maneuvers) or (self.use_transformer and self.use_grid_soc):
			...
			grid_enc = self.maxpool(self.leaky_relu(self.conv2(self.leaky_relu(self.conv1(hist_grid)))))
			grid_enc = torch.squeeze(grid_enc) # [128, 16, 5]
			grid_enc = grid_enc.permute(1,0,2) # [16, 128, 5]
			grid_emb = self.grid_emb(grid_enc) # [16, 128, 32]
			grid_a, (grid_enc,_) = self.enc_lstm(self.leaky_relu(grid_emb)) # [2, 128, 64]
			grid_enc = self.proj_grid(grid_enc) # [2, 128, 80]
			...
			if self.use_bidir:
				# XXX enc = torch.cat((soc_enc, hist_enc[0, :, :]),1)
				enc = torch.cat((grid_enc[0, :, :], hist_enc[0, :, :]),1)
				self.hist_enc_back = hist_enc[1, :, :]
				self.grid_enc_back = grid_enc[1, :, :]
			else:
				#enc = torch.cat((soc_enc, hist_enc),1)
				enc = torch.cat((grid_enc, hist_enc),1)
