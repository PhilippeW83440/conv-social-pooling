...
6: new feature: Speed and/or Accel

With offset newFeats:
6: Lane Id
7: Lateral maneuver
...
for ii = 1:N
	...
	for k = 1:length(traj{ii}(:,1));
		...
		ind = ind(1);
		lane = traj{ii}(k,newFeats + 6);
		
		
		% Get lateral maneuver:
		ub = min(size(vehtraj,1),ind+40);
		lb = max(1, ind-40);
		if vehtraj(ub,newFeats + 6)>vehtraj(ind,newFeats + 6) || vehtraj(ind,newFeats + 6)>vehtraj(lb,newFeats + 6)
			traj{ii}(k,newFeats + 7) = 3;
		elseif vehtraj(ub,newFeats + 6)<vehtraj(ind,newFeats + 6) || vehtraj(ind,newFeats + 6)<vehtraj(lb,newFeats + 6)
			traj{ii}(k,newFeats + 7) = 2;
		else
			traj{ii}(k,newFeats + 7) = 1;
		end
		...
		frameEgo = timetraj{time}(timetraj{time}(:,newFeats + 6) == lane,:);
		frameL = timetraj{time}(timetraj{time}(:,newFeats + 6) == lane-1,:);
		frameR = timetraj{time}(timetraj{time}(:,newFeats + 6) == lane+1,:);
		...
		if ~isempty(frameL)
			for l = 1:size(frameL,1)
				...
				%if abs(y) <90
					...
					traj{ii}(k,newFeats + 8+gridInd) = frameL(l,2);
				%end
			end
		end
		for l = 1:size(frameEgo,1)
			...
			%if abs(y) <90 && y~=0
				...
				traj{ii}(k,newFeats + 8+gridInd) = frameEgo(l,2);
			%end
		end
		if ~isempty(frameR)
			for l = 1:size(frameR,1)
				...
				%if abs(y) <90
					...
					traj{ii}(k,newFeats + 8+gridInd) = frameR(l,2);
				%end
			end
		end
...
for k = 1:N
	...
	for l = 1:length(carIds)
		vehtrack = trajSet(trajSet(:,2) ==carIds(l),3:5+newFeats)';
